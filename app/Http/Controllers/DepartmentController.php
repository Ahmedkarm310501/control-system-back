...
class DepartmentController extends Controller
{
    ...
    public function getCoursesInDepartmentForUser(Request $request, UserService $userService)
    {
        $department_id = $request->dept_id;
        $user_id = $request->user_id;
        $courses = $userService->getCoursesInDepartmentForUser($department_id, $user_id);
        if (!$courses) {
            return $this->error('No Courses Found', 404);
        }
        return $this->success($courses, 200, 'all Courses');
